...
let XHWLHttpURL: String = "http://10.51.22.43:8080/community"
    ...
    func postHttpTool(_ parameters: Parameters) {
        ...
        Alamofire.request(requestUrl, method: .post, parameters: parameters, encoding: URLEncoding.default)
            .responseJSON { response in
                ...
                //                print(response.response) // HTTP URL response
                //                print(response.data)     // server data
                //responseprint(response.result)   // result of response serialization
...
//                    multipartFormData.append
    ...
    func uploadHttpTool(_ data: [Data], _ isVideo:Bool) {
...
//                   image/jpg,image/png 视频:video/quicktime
                    if isVideo {
                        multipartFormData.append(data[i], withName:"files", fileName: "files.mp4", mimeType: "video/mpeg4")
                    } else {
                        multipartFormData.append(data[i], withName:"files", fileName: "files.png", mimeType: "image/png")
                    }
                }
        },
            to: requestUrl,
