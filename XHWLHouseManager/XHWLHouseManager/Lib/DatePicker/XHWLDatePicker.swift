...
protocol XHWLDatePickerDelegate:NSObjectProtocol {
    func datePicker(_ datePicker:XHWLDatePicker, dateDidChange date:Date)
    func datePicker(_ datePicker:XHWLDatePicker, didCancel date:UIButton)
    func datePicker(_ datePicker:XHWLDatePicker, didSelectedDate date:Date)
}

class XHWLDatePicker: UIControl, UIScrollViewDelegate {
    
    enum ScrollViewTagValue:Int {
        case days = 1
        case months
        case years
        case hours
    ...
    var minimumDate:Date! {
        willSet {
            let componentsMin:DateComponents = self.calendar.component(.year|.month|.day, from: newValue)
            let yearMin:NSInteger = componentsMin.year //[componentsMin year];
            self.minYear = yearMin
            setupControl()
        }
    }
...
//            maximumDate = newValue
    ...
    var datePickerMode:XHWLDatePickerMode!
    var locale:Locale! {
        get {
            if (!_locale) {
    ...
    var timeZone:TimeZone! {
        get {
            if (!_timeZone) {
                _timeZone = TimeZone.init(abbreviation: "CCD")
            }
            return _timeZone
        }
    }
    var isOpen:Bool!
    weak var delegate:XHWLDatePickerDelegate?
    
    
    // Data of years, months, days, dates, hours, minutes, seconds
    var years, days, dates:NSMutableArray
    var months:NSMutableArray {
        get {
            let monthAry:NSMutableArray = NSMutableArray()
            for (var monthNumber:int = 1; monthNumber <= 12; monthNumber++) {
                let dateString:String = "\(monthNumber)月"
                monthAry.add(dateString)
            }
            months = monthAry
            return months
        }
    ...
    var hours:NSMutableArray {
        get {
            if (!hours) {
                let hourAry:NSMutableArray = NSMutableArray()
                
                for (var i:int = 0; i < 24; i++) {
                    if (i < 10) {
                        hourAry.add([NSString stringWithFormat:@"0%d%@", i, LocalLanguage(@"shid")])
                    } else {
                        hourAry.add([NSString stringWithFormat:@"%d%@", i, LocalLanguage(@"shid")])
                    }
                }
                hours = hourAry
            }
    ...
    var minutes:NSMutableArray {
        if (!minutes) {
            let minuteAry:NSMutableArray = NSMutableArray()
            /**
             *  only show mininute 00 10 20 ~ 50 in hotel tour edit order for flight pick up
             */
            for (int i = 0; i < 60; i++) {
                if (!i) {
                    minuteAry.add([NSString stringWithFormat:@"%d0%@", i, LocalLanguage(@"fen")])
                } else {
                    minuteAry.add([NSString stringWithFormat:@"%d%@", i, LocalLanguage(@"fen")])
                }
    ...
    var seconds:NSMutableArray  {
        if (!seconds) {
            ...
            let secondAry:NSMutableArray = NSMutableArray()
            
            for (var i:int = 0; i < 60; i++) {
                if (i < 10) {
                    let str = [NSString stringWithFormat:@"0%d", i]
                    secondAry.add(str)
                } else {
                    secondAry.add([NSString stringWithFormat:@"%d", i])
                }
            }
            _seconds = secondAry
        }
    ...
    var scrollViewYears, scrollViewMonths, scrollViewDays, scrollViewDates, scrollViewHours, scrollViewMinutes,scrollViewSeconds:UIScrollView
...
//            tintColor = newValue
    ...
    var dateFormatter:DateFormatter! {
        get {
            if (!dateFormatter) {
                let dateFormatter:DateFormatter = Date.shareDateFormatter
...
//    #pragma mark - Initializers
    ...
    init(superView:UIView) {
        super.init(frame: CGRect(x:0.0, y:superView.frame.size.height, width:superView.frame.size.width, height:kHooDatePickerHeight))
        ...
        self.addSubview(self.headerView) // toolbar
        setupControl()
    }
...
//    }
    ...
    func setupControl() {
        ...
        // Clear old selectors
        removeSelectorYears()
        removeSelectorMonths()
        removeSelectorDays()
        removeSelectorHours()
        removeSelectorMinutes()
        removeSelectorSeconds()
        
        // Generate collections days, months, years, hours, minutes and seconds :
        _years = getYears()
        _days = [self getDaysInMonth:Date()];
...
//    #pragma mark - Build Selector Months
    ...
    func buildSelectorLabelsMonths() {
...
//    #pragma mark - Build Selector Minutes
    ...
    func buildSelectorLabelsMinutes() {
    ...
    [years addObject:[NSString stringWithFormat:@"%ld%@", (long)i, LocalLanguage(@"nian")]];
    }
    ...
    [days addObject:[NSString stringWithFormat:@"%d%@", i, LocalLanguage(@"ri")]]; //
    }
    ...
    [sureButton setImage:[FireflyImageManager imageNamed:@"base_search_time_select"] forState:UIControlStateNormal];
    ...
    [cancelButton setImage:[FireflyImageManager imageNamed:@"base_search_time_delete"] forState:UIControlStateNormal];
    ...
    make.right.equalTo(sureButton.mas_left).with.offset(-15);
    ...
    _titleLabel.text = LocalLanguage(@"xzrq");// 选择日期
    ...
    make.left.equalTo(_headerView).with.offset(15);
    make.centerY.equalTo(_headerView);
    }];
    _headerView.backgroundColor = [UIColor colorWithRed:247/255.0 green:98/255.0 blue:64/255.0 alpha:1];
    }
    return _headerView;
    }
